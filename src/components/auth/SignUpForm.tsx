        ...
        <div className="flex flex-col justify-center w-64 mx-auto">
            <form onSubmit={handleSubmit(signUpWithCredentials)}>
                <div className="form-control mb-2">
                    ...
                    <input type="text" className={`input input-bordered rounded ${errors.email ? "border-error" : "border-base-300"}`} {...register("email")} />
                    {errors.email && <p className="text-error text-xs">A valid email address is required.</p>}
                </div>
                <div className="form-control mb-2">
                    <label className="label">Password</label>
                    <input type="password" className={`input input-bordered rounded ${errors.password ? "border-error" : "border-base-300"}`} {...register("password")} />
                    {errors.password && <p className="text-error text-xs">Password must contain between 6 and 18 characters.</p>}
                </div>
                <button className="btn btn-primary normal-case w-full flex mx-auto justify-center items-center gap-4 my-5 rounded">
            ...
            <button onClick={SignUpWithGoogle} className="btn bg-base-300 normal-case w-full flex mx-auto justify-center items-center gap-4 mb-2 rounded">
